...
from .record_ads import sort_ad_events,findPackage_droidbot,contain_ad_text
...
MAX_SCROLL_NUM = 7
POLICY_AD_DFS='ad_dfs'
POLICY_AD_BFS='ad_bfs'
POLICY_DFS='dfs'
POLICY_BFS='bfs'
...
class MonkeyPolicy(InputPolicy):
    def __int__(self,device,app):
        super(MonkeyPolicy, self).__init__(device, app)
        self.device=device
        self.app=app
    def generate_event(self,input_manager):
        """
    ...
    def get_current_activity(self):
        ...
        # Extracting package name and activity
        # Expected format: mResumedActivity: ActivityRecord{... u0 package_name/activity_name ...}
        try:
            split_result = result.split()
            activity_info = split_result[3]  # This should contain package_name/activity_name
            package_name, activity_name = activity_info.split('/')
        except (IndexError, ValueError):
            package_name = "Error: Could not extract package name"
            activity_name = "Error: Could not extract activity name"
    ...
    def get_ads(self):
        ...
        if state!=None:
            ...
            if any("com.android.vending" in activity for activity in self.current_state.activity_stack):
                # if 'com.android.vending/com.google.android.finsky.activities.MainActivity' in current_state.activity_stack:

                findPackage_droidbot(self.current_state, self)
        # # package_name, activity_name=self.get_current_activity()
        # if package_name=='com.android.vending':
        #     # if 'com.android.vending/com.google.android.finsky.activities.MainActivity' in current_state.activity_stack:
        #
        #     findPackage_droidbot(current_state, self)
...
class UtgNaiveSearchPolicy(UtgBasedInputPolicy):
    ...
    def __init__(self, device, app, random_input, search_method):
        ...
        self.ad_buttons=['install','play','ad','start','download','order','spin','connect','get','open','learn more','try']
    def generate_event_based_on_utg(self,input_manager):
        """
    ...
    def select_a_view(self, state):
        ...
        for view in views:
            ...
            if view_text in self.ad_buttons \
...
class UtgGreedySearchPolicy(UtgBasedInputPolicy):
    ...
    def __init__(self, device, app, random_input, search_method,using_ad_keywords=True):
        super(UtgGreedySearchPolicy, self).__init__(device, app, random_input)
        self.logger = logging.getLogger(self.__class__.__name__)
        self.search_method = search_method
        self.using_ad_keywords=using_ad_keywords
        self.preferred_buttons = ["yes", "ok", "activate", "detail", "more", "access",
                                  "allow", "check", "agree", "try", "go", "next"]

        self.ad_buttons=['install','play','ad','start','download','order',
                         'spin','connect','get','open','more','try','now',
                         'install now','download now','play now','start now','order now',
                         'more apps','more app','try now']
        # self.ad_buttons = [button.lower().strip() for button in self.ad_buttons]  # Normalize the ad button texts
    ...
    def generate_event_based_on_utg(self,input_manager):
        ...
        # If jump to google play
        if any("com.android.vending" in activity for activity in current_state.activity_stack):
            # if 'com.android.vending/com.google.android.finsky.activities.MainActivity' in current_state.activity_stack:
        ...
        #get_llm_events: get scrollable
        #sort_llm_events()

        if self.using_ad_keywords==True:
            possible_events=sort_ad_events(possible_events,self.ad_buttons)
        ...
        for input_event in possible_events:
            if not self.utg.is_event_explored(event=input_event, state=current_state):
                # event_str = input_event.get_event_str(state=current_state)
...
class UtgAdGreedySearchPolicy(UtgBasedInputPolicy):
    """
    DFS/BFS (according to search_method) strategy to explore UFG (new)
    """

    def __init__(self, device, app, random_input, search_method):
        super(UtgAdGreedySearchPolicy, self).__init__(device, app, random_input)
        self.logger = logging.getLogger(self.__class__.__name__)
        self.search_method = search_method
    ...
    def generate_event_based_on_utg(self,input_manager):
        ...
        # possible_events=current_state.get_possible_input()
        # if not contain_ad_text(possible_events,self.ad_buttons):
            ...
            #     possible_events.insert(0,action)
            # pass
        #get_llm_events: get scrollable
        ...
        if len(scrollable_views) > 0:
            ...
            # possible_events = current_state.get_possible_input()
            # if contain_ad_text(whole_state_actions, self.ad_buttons):
    ...
    def _make_prompt_lmql(self, state_prompt, action_history, is_text, state_str, view_text=None, thought_history=None,
        ...
        # query_template="""You are a smartphone assistant to help navigate through a mobile app and click the in-app advertisements.
        # Previous UI actions: {history}
        # Current UI State:{ui_desc}
        # Tell me the UI element that you think should contain advertisements?
        # Or if there is no advertisement, which action should you choose to navigate the app.
        # - id=[ID] - action=[ACTION] - input text=[INPUT_TEXT].
        #     """
        # prompt=query_template.format(history='\n'.join(history_with_thought),ui_desc=state_prompt)
