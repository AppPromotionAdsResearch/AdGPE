...
from .input_policy import UtgBasedInputPolicy, UtgNaiveSearchPolicy, UtgGreedySearchPolicy, \
                         UtgReplayPolicy, UtgAdGreedySearchPolicy,\
                         ManualPolicy, \
                         POLICY_NAIVE_DFS, POLICY_GREEDY_DFS,POLICY_DFS,POLICY_BFS, \
                         POLICY_NAIVE_BFS, POLICY_GREEDY_BFS, \
                         POLICY_REPLAY, POLICY_MEMORY_GUIDED, POLICY_LLM_GUIDED, \
                         POLICY_MANUAL, POLICY_MONKEY, POLICY_NONE,POLICY_AD_BFS,POLICY_AD_DFS,MonkeyPolicy
...
class InputManager(object):
    ...
    def get_input_policy(self, device, app, master):
        ...
        elif self.policy_name in [POLICY_AD_BFS,POLICY_AD_DFS]: #llm
            input_policy=UtgAdGreedySearchPolicy(device,app,self.random_input,self.policy_name)
        elif self.policy_name in [POLICY_BFS,POLICY_DFS]: # no ad keywords
            input_policy=UtgGreedySearchPolicy(device,app,self.random_input,self.policy_name,using_ad_keywords=False)
        else:
    ...
    def start(self):
        ...
        try:
            if self.policy is not None:
                ...
                self.policy=MonkeyPolicy(device=self.device,app=self.app)
                throttle = self.event_interval * 500
                # os.system("adb shell settings put global policy_control immersive.full=*")
                monkey_cmd = "adb -s %s shell monkey %s --pct-touch 100 --ignore-crashes   --ignore-security-exceptions" \
                             " --throttle %d -v %d" % \
                             (self.device.serial,
